...
while True:
    ...
    for face in faces:
        ...
        # Get the eye regions
        left_eye_points = np.array(landmarks_points[36:42], np.int32)
        right_eye_points = np.array(landmarks_points[42:48], np.int32)

        # Fill the eye regions with white
        cv2.fillPoly(frame, [left_eye_points], (255, 255, 255))
        cv2.fillPoly(frame, [right_eye_points], (255, 255, 255))
        ...
        # Display name, age, and gender below the face
        x, y, w, h = face.left(), face.top(), face.width(), face.height()
        cv2.putText(frame, f"Name: {name}", (x, y + h + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(frame, f"Age: {age}", (x, y + h + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(frame, f"Gender: {gender}", (x, y + h + 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

    # Display the resulting frame
    cv2.imshow('Face with Multicolored Grid, White Points, and White Eyes', frame)
